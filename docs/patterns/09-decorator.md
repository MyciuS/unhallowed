...
- **Code:** `src/Unhallowed.Core/Entities/Stats/`
- **Demo:** `dotnet run --project src/Unhallowed.PatternDemos -- decorator`
- **Status:** implemented
...
Item pickups wrap IPlayerStats. Ten items produce a ten-link chain and no change to Player.
...
![before](../uml/09-decorator-before.png)

_Export from MagicDraw (requirement 14)._

## 4. UML class diagram - AFTER

![after](../uml/09-decorator-after.png)
...
```csharp
// src/Unhallowed.Core/Entities/Stats/
```
...
```
dotnet run --project src/Unhallowed.PatternDemos -- decorator
```
