...
- **Code:** `src/Unhallowed.Client/Rendering/GdiRenderSurface.cs`
- **Demo:** `dotnet run --project src/Unhallowed.PatternDemos -- adapter`
- **Status:** implemented
...
GdiRenderSurface adapts System.Drawing.Graphics to the IRenderSurface the domain draws through.
...
```csharp
// src/Unhallowed.Client/Rendering/GdiRenderSurface.cs
```
...
```
dotnet run --project src/Unhallowed.PatternDemos -- adapter
```
