...
- **Code:** `src/Unhallowed.Persistence/EnemyDefinitionRepository.cs (data source)`
- **Demo:** `not registered yet - add it to src/Unhallowed.PatternDemos/PatternCatalog.cs`
- **Status:** planned

> Not implemented yet. Start by reading the "Where it goes" note below.
...
An item and enemy prototype registry loaded once from SQLite, then cloned per spawn instead of re-reading the database.
...
![before](../uml/05-prototype-before.png)

_Export from MagicDraw (requirement 14)._

## 4. UML class diagram - AFTER

![after](../uml/05-prototype-after.png)
...
```csharp
// src/Unhallowed.Persistence/EnemyDefinitionRepository.cs (data source)
```
...
```
not registered yet - add it to src/Unhallowed.PatternDemos/PatternCatalog.cs
```
