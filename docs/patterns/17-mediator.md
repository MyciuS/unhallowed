...
- **Code:** `src/Unhallowed.Core/Simulation/ (to add)`
...
A CombatMediator coordinating player, enemy, projectile and pickup interactions so entities never reference each other directly.
...
![before](../uml/17-mediator-before.png)

_Export from MagicDraw (requirement 14)._

## 4. UML class diagram - AFTER

![after](../uml/17-mediator-after.png)
...
```csharp
// src/Unhallowed.Core/Simulation/ (to add)
```
