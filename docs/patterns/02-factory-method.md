...
- **Code:** `src/Unhallowed.Core/Rooms/RoomTypes.cs (current call site)`
...
An EnemyFactory that turns an archetype name into a configured Enemy, replacing the direct `new Enemy(...)` calls in room population.
...
![before](../uml/02-factory-method-before.png)

_Export from MagicDraw (requirement 14)._

## 4. UML class diagram - AFTER

![after](../uml/02-factory-method-after.png)
...
```csharp
// src/Unhallowed.Core/Rooms/RoomTypes.cs (current call site)
```
