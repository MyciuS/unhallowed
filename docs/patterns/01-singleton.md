...
- **Code:** `src/Unhallowed.Core/Common/ServiceRegistry.cs`
- **Demo:** `dotnet run --project src/Unhallowed.PatternDemos -- singleton`
- **Status:** implemented
...
One service registry per process, holding the event bus and shared game services.
...
![before](../uml/01-singleton-before.png)

_Export from MagicDraw (requirement 14)._

## 4. UML class diagram - AFTER

![after](../uml/01-singleton-after.png)
...
```csharp
// src/Unhallowed.Core/Common/ServiceRegistry.cs
```
...
```
dotnet run --project src/Unhallowed.PatternDemos -- singleton
```
