...
- **Code:** `src/Unhallowed.Core/Entities/ (to add)`
...
A damage pipeline: invulnerability, then shields, then armour, then health, each link able to absorb the hit and stop the chain.
...
![before](../uml/13-chain-of-responsibility-before.png)

_Export from MagicDraw (requirement 14)._

## 4. UML class diagram - AFTER

![after](../uml/13-chain-of-responsibility-after.png)
...
```csharp
// src/Unhallowed.Core/Entities/ (to add)
```
